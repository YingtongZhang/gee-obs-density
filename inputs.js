/**
 * @fileoverview Functions to build and process Landsat and Sentinel collections
 * for display in time series viewer or analysis
 * 
 * @author Eric Bullock <Eric.Bullock@usda.gov> 
...
function getLandsat(options) {
  ...
  var targetBands = (options && options.targetBands) || ['BLUE','GREEN','RED','NIR','SWIR1','SWIR2','TEMP','NBR','NDFI','NDVI','GV','NPV','Shade','Soil', 'EVI', 'EVI2', 'BRIGHTNESS', 'GREENNESS', 'WETNESS']
  ...
  if (useMask) {
    ...
    collection8 = collection8.map(function(i) {return i.select(bandListL8).rename(nameListL8)})
    collection4 = collection4.map(function(i) {return i.select(bandListL457).rename(nameListL457)})
    collection5 = collection5.map(function(i) {return i.select(bandListL457).rename(nameListL457)})
    collection7 = collection7.map(function(i) {return i.select(bandListL457).rename(nameListL457)})
  }
...
function prepareL4L5(image){
  ...
  if (maskBool){
    ...
    var mask4 = image.select(bandList).reduce(ee.Reducer.max()).lt(10000)
    // Mask hazy pixels
    var mask5 = (image.select("sr_atmos_opacity").unmask(-1)).lt(300) //AOD threshold
    // var mask5 = (image.select("sr_atmos_opacity")).lt(300)
...
function prepareL8(image){
  ...
  if (maskBool){
    ...
    var mask3 = image.select(bandList).reduce(ee.Reducer.min()).gt(0)
    var mask4 = image.select(bandList).reduce(ee.Reducer.max()).lt(10000)
    var mask5 = ee.Image(image).select(['sr_aerosol']).remap(validTOA, ee.List.repeat(1, validTOA.length), 0)
